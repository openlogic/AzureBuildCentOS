...
lis="4.2.5"
kversion=$( curl -s "http://vault.centos.org/6.9/updates/Source/SPackages/" | \
            grep "kernel-2.6.32-696" | \
...
curl -so ${topdir}/kernel-${kversion}.src.rpm "http://vault.centos.org/6.9/updates/Source/SPackages/kernel-${kversion}.src.rpm"
...
rm -f LIS-${lis}_linux-${kbasever}.patch
